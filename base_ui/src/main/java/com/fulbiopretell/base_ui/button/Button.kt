package com.fulbiopretell.retoyape.base_ui.button
...
import com.fulbiopretell.retoyape.base_ui.R
import com.fulbiopretell.retoyape.base_ui.databinding.ButtonBinding
import com.fulbiopretell.retoyape.core.extensions.setSafeOnClickListener
import com.fulbiopretell.retoyape.core.extensions.setSvgColor
import com.fulbiopretell.retoyape.core.helpers.StylesHelper
...
class Button: LinearLayout {
    ...
    val typefaceRegular = ResourcesCompat.getFont(context, R.font.helvetica)
    ...
    var iconLeft: Int = -1
    var iconRight: Int = -1
    var color: Int = ContextCompat.getColor(context, com.fulbiopretell.retoyape.core.R.color.core_black_color)
    var rippleColor: Int = ContextCompat.getColor(context, com.fulbiopretell.retoyape.core.R.color.core_gray_3)
    var textColor: Int = ContextCompat.getColor(context, com.fulbiopretell.retoyape.core.R.color.core_white_color)
    ...
    private var colorDisable: Int = ContextCompat.getColor(context, com.fulbiopretell.retoyape.core.R.color.core_gray_3)
    var textWithUnderline: Boolean = false
    private var textColorDisable: Int = ContextCompat.getColor(context, com.fulbiopretell.retoyape.core.R.color.core_gray_4)
    ...
    private fun setupText() {
        ...
        if (textWithUnderline) {
            binding.textButton.paintFlags = binding.textButton.paintFlags or Paint.UNDERLINE_TEXT_FLAG
        }
        ...
        stylesHelper.updateTextViewStyle(binding.textButton, textStyle, textColorStyle, this.textSize)

        textBtn?.let {
            binding.textButton.text = textBtn
        }
    }
    ...
    private fun setBackgroundOutlineDisable() {
        ...
        binding.textButton.setTextColor(textColorDisable)
    ...
    private fun setBackgroundOutline() {
        ...
        binding.textButton.setTextColor(borderColor)
    ...
    fun setupIcon(color: Int) {
        ...
        if (iconLeft == -1) {
            binding.iconButtonLeft.visibility = View.GONE
        } else {
            binding.iconButtonLeft.visibility = View.VISIBLE
            binding.iconButtonLeft.setImageDrawable(ContextCompat.getDrawable(context, iconLeft))
            binding.iconButtonLeft.setSvgColor(color)
        }

        if (iconRight == -1) {
            binding.iconButtonRight.visibility = View.GONE
        } else {
            binding.iconButtonRight.visibility = View.VISIBLE
            binding.iconButtonRight.setImageDrawable(ContextCompat.getDrawable(context, iconRight))
            binding.iconButtonRight.setSvgColor(color)
        }
    }
    ...
    fun ilimitedMaxLines() {
        binding.textButton.maxLines = Int.MAX_VALUE
        setupText()
    }

    fun maxLines(maxLines: Int) {
        binding.textButton.maxLines = maxLines
        setupText()
    }
    ...
    fun resetStyleColorDisable() {
        this.colorDisable = ContextCompat.getColor(context, com.fulbiopretell.retoyape.core.R.color.core_gray_3)
        this.textColorDisable = ContextCompat.getColor(context, com.fulbiopretell.retoyape.core.R.color.core_gray_4)
        setupUI()
    }
